...
// In order to follow clean-up procedures, we can use the @finally preffix
{
  let promise = new Promise(function(resolve, reject) {
    // ...successful call:
    setTimeout(() => resolve("3) done!"), 1000); 
    // ...rejected call:
    // setTimeout(() => reject("2) error!"), 1000);
  });

  promise.then(
    script => console.log(`3) done!`),
    error => console.error(`3) error`)
  );

  promise.finally(script => console.log('3) Last handler @finally being processed!'));
...
*/
{
  function loadScript(src) {
    return new Promise(function(resolve, reject) {
      ...
      script.onerror = () => reject(new Error(`4) Script load error for ${src}`));
  ...
  promise.then(
    script => console.log(`4) Loading: ${script.src}`),
    error => console.error(`4) Error: ${error.message}`)
  );
