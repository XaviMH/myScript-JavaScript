...
// asynchronously should provide a callback argument where we put the function to run after it’s complete.
{
  function loadScript(src, callback) {
    console.log(`1) Now attempting to read script ${src}...`)
  ...
  loadScript('https://cdnjs.cloudflare.com/ajax/libs/lodash.js/3.2.0/lodash.js', script => {
    console.log(`1) Cool, the script ${script.src} is loaded`);
    //console.log( _ ); 
  });
...
{
  function loadScript(src, callback) {
    console.log(`2) Now attempting to read script ${src}...`)
    let script = document.createElement('script');
    script.src = src;
  ...
  loadScript('https://cdnjs.cloudflare.com/ajax/libs/lodash.js/3.2.0/lodash.js' , function(error, script) {
    if (error) {
      console.error(`2.1) Error! For some reason, the script ${script.src} could not be loaded`);
    } else {
      console.log(`2.1) Cool, the script ${script.src} is loaded (again!)`);
  ...
  loadScript('1.js' , function(error, script) {
    if (error) {
      console.error(`2.2) Error! For some reason, the script "1.js" could not be loaded`);
    } else {
      console.log(`2.2) Cool, the script ${script.src} is loaded (again!)`);
      //console.log( _ ); // _ is a function declared in the loaded script
    }
  });
...
{
  function loadScript(src, callback) {
    console.log(`3) Now attempting to read script ${src}...`)
    let script = document.createElement('script');
    script.src = src;
  ...
  loadScript('11.js', function(error, script) {
    if (error) {
      console.error(`3) ERROR: issue loading "11.js"`);
      console.error(`3) ERROR: The error seems to be >>>> ${error}`);
    } else {
      // ...
      loadScript('22.js', function(error, script) {
        if (error) {
          console.error("3) ERROR: Script 22.js does not exist!");
        } else {
          // ...
          loadScript('33.js', function(error, script) {
            if (error) {
              console.error("3) ERROR: Script 33.js does not exist!");
            } else {
