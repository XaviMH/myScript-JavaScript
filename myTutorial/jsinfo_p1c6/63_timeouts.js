...
// Set and use timeouts can be done in different ways. A few examples follow
{
    ...
    abc = setTimeout(sayHi, 1000, "Hello", "John");  // Hello, John
    clearTimeout(abc);                               // stops the timeout "abc"
}

/* Theory 2 */
// Note that the descripotion of the function accepts a string, but don't use that, just replace it
// with a function arrow, like so:
{
    ...
    abc = setTimeout(() => alert('Hello'), 1000);  // Hello
    clearTimeout(abc);                             // stops the timeout "abc"
}
